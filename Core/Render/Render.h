	...
	for (int i = 0; i < PlayerArray.NumElements; i++)
	{
		...
		if (Settings::Visuals::Enable)
		{
			...
			if (Settings::Visuals::Weapon)
			{
				...
				{
					Canvas->K2_DrawText(DisplayName, FVector2D(BottomMiddle.X, MostBottom + BottomTextOffset - 2), Settings::Visuals::FontSize, RenderColor, true, false, Settings::Visuals::TextOutline);
				}
				else
				{
					Canvas->K2_DrawText(DisplayName, FVector2D(BottomMiddle.X, MostBottom + BottomTextOffset - FontSize + 5), Settings::Visuals::FontSize, RenderColor, true, false, Settings::Visuals::TextOutline);
				}
											...
											if (StaticClasses::KismetMathLibrary)
											{
												if (SDK::Classes::CameraLocation && SDK::Classes::CameraRotation && AimbotPosition)
												{
													...
													if (TargetRotation)
													{
														...
														if (PlayerController)
														{
															bool AimbotKeyDown = GetAsyncKeyState(Settings::Combat::Keybind);
															if (AimbotKeyDown)
															{
