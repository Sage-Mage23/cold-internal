...
namespace Custom
{
	...
	auto IsPlayerShotable(FVector Location, FVector Bone) -> bool
	{
		if (Location.X >= Bone.X - 20 && Location.X <= Bone.X + 20 && Location.Y >= Bone.Y - 20 && Location.Y <= Bone.Y + 20 && Location.Z >= Bone.Z - 30 && Location.Z <= Bone.Z + 30) return true;
		else return false;
	}
